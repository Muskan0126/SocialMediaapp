...
""" This file contains all the models for the post app
The models are used to define the database schema for the post app
The models are used to define the post model, likes model, comment model, notification model, follow model, stream model, and story model
"""
...
def post_upload_path(instance, filename):
    ...
    return (
        f"https://instagram-clone-muskan.s3.ap-southeast-2.amazonaws.com/posts/{instance.user.id}/{uuid.uuid4()}{ext}"
    )
...
def story_upload_path(instance, filename):
    ext = os.path.splitext(filename)[1]
    return (
        f"https://instagram-clone-muskan.s3.ap-southeast-2.amazonaws.com/story/{instance.user.id}/{uuid.uuid4()}{ext}"
    )
...
class Comment(models.Model):
    ...
    liked_by_user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="liked_comment"
    )
...
class Story(models.Model):
    ...
    image = models.ImageField(upload_to=story_upload_path)
